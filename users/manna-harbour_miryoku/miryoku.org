...
#+BEGIN_SRC python :var alphas_table=colemakdh :var thumbs_table=thumbs :var hold_table=hold :var no_hold="" :var symbol_names_table=symbol-names :var mods_table=mods :var nonkc_table=nonkc :tangle no :results verbatim
width = 19
...
for tap_row, hold_row in zip(alphas_table + thumbs_table, hold_table):
  results += '    '
  for idx, (tap, hold) in enumerate(zip(tap_row, hold_row)):
    if tap == '':
      code = 'U_NU'
    ...
    if not str(code).startswith(nonkc_tuple):
      code = 'KC_' + str(code)
    if no_hold == 'L' and idx >= 5 or no_hold == 'R' and idx < 5:
      code = 'KC_TRNS'
    elif no_hold == '':
...
#+NAME: table-enums
#+BEGIN_SRC python :var hold_table=hold :var mods_table=mods :var extra_layers="" :tangle no
mods_dict = dict.fromkeys(mods_table[0])
results = 'enum layers { BASE, ' + ( extra_layers + ', MBO, ' if extra_layers else 'MBO, ' )
...
<<table-enums(extra_layers="MIRYOKU_EXTRA_LAYERS")>>
#else
<<table-enums()>>
#endif
...
#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemak, no_hold="LR")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdhk, no_hold="LR")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=dvorak, no_hold="LR")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=halmak, no_hold="LR")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=workman, no_hold="LR")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=qwerty, no_hold="LR")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdh, no_hold="LR")>>

#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemak, no_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdhk, no_hold="R")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=dvorak, no_hold="R")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=halmak, no_hold="R")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=workman, no_hold="R")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=qwerty, no_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdh, no_hold="R")>>

#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemak, no_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdhk, no_hold="L")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=dvorak, no_hold="L")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=halmak, no_hold="L")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=workman, no_hold="L")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=qwerty, no_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdh, no_hold="L")>>
...
#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="LR")>>

#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="R")>>

#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, no_hold="L")>>
...
To build for any keyboard using this layout (boardsource/5x12,
cannonkeys/ortho60, fractal, jj50, jnao, keebio/nyquist, keycapsss/o4l_5x12,
preonic, rgbkb/zygomorph),
